    ...
    if _mongodb_client is None:
        ...
        if not mongo_uri:
            ...
            if pwd:
                ...
                symbol_logger.warning("No MongoDB credentials found in environment, using local connection")
...
def _load_symbols_from_mongodb():
    ...
    symbol_logger.info(f"Successfully loaded {sum(len(symbols) for symbols in mongodb_symbols.values())} symbols from MongoDB")
...
def _load_symbol_categories_from_mongodb():
    ...
    symbol_logger.info(f"Successfully loaded {len(mongodb_categories)} symbol categories from MongoDB")
