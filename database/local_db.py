...
pwd = getenv(
    "MONGODB_PWD"
)  # This environment variable is set in the system environment, not in the .env file
...
class Database:
    def __init__(self, col, delete=False):
        ...
        self.client = MongoClient(
            f"mongodb+srv://sammy:{pwd}@cluster1.565lfln.mongodb.net/?retryWrites=true&w=majority"
        )
    ...
    def change_type(self, col: str):
        ...
        if update_result.modified_count == 0:
            local_db_logger.error(
                f"Error: Document with _id {doc['_id']} was not updated."
            )
    ...
    def add_doc(self, doc: dict, col: str):
        ...
        try:
            ...
            local_db_logger.exception(
                f"Failed to add document to MongoDB's {col} collection. Error:"
            )
            return False
    ...
    def change_tv_links(self, col: str):
        ...
        if bulk_operations:
            try:
                ...
                print(
                    f"Bulk write operation completed. Matched: {result.matched_count}, Modified: {result.modified_count}"
                )
            except Exception as e:
    ...
    def delete_some(self, count: int, col: str):
        ...
        latest_100_ids = [
            ...
            .limit(count)
        ]  # Get the IDs of the latest 100 documents
        result = self.db[col].delete_many(
            {"_id": {"$nin": latest_100_ids}}
        )  # Delete all documents that are not in the latest 100
        local_db_logger.info(
            f"Deleted {result.deleted_count} documents from {col} collection."
        )
    ...
    def get_entries_in_timespan(
        ...
        try:
            ...
            return self.db[col].find(
                {
                    "unixTime": {"$gte": start_time * 1000, "$lte": end_time * 1000},
                    "category": category,
                }
            )
        except Exception as e:
    ...
    def unix_to_readable(self, unix_timestamp):
        ...
        timestamp_datetime_kolkata = timestamp_datetime.replace(
            ...
            timezone
        )  # Format the datetime object into a short, readable string
