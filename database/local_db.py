...
class Database:
        ...
        try:
            ...
            if not mongo_uri:
                ...
                if pwd:
                    ...
                    local_db_logger.warning(
                        "No MongoDB credentials found in environment, returning False"
                    )
                    return
            ...
            local_db_logger.exception(
                f"Failed to connect to MongoDB database. Error: {e}"
            )
            return
    ...
    def add_doc(self, doc: dict):
        ...
        try:
            ...
            local_db_logger.info(
                f"Successfully sent a doc to {self.collection_name} collection!"
            )
            return True
        except Exception as e:
            local_db_logger.exception(
                f"Failed to add document to MongoDB's {self.collection_name} collection. Error:"
            )
            return False
    ...
    def change_tv_links(self):
        ...
        try:
            ...
            if bulk_operations:
                ...
                local_db_logger.info(
                    f"Successfully updated TV links in {self.collection_name} collection. Modified: {result.modified_count}"
                )
            else:
                local_db_logger.info(
                    f"No TV links to update in {self.collection_name} collection"
                )
    ...
    def extract_img_src(self, url: str, t) -> str:
        ...
        try:
            ...
            if img_tag and "src" in img_tag.attrs:
                ...
                local_db_logger.debug(
                    f"No img tag with class 'tv-snapshot-image' found in {url}."
                )
                return ""
        except Exception as e:
            local_db_logger.error(
                f"Error fetching or parsing the URL {url} for {t}: {e}"
            )
            return ""
    ...
    def delete_all(self):
        ...
        try:
            ...
            local_db_logger.info(
                f"Deleted {result.deleted_count} documents from {self.collection_name} collection"
            )
        except Exception as e:
    ...
    def delete_some(self, count: int):
        ...
        try:
            ...
            if latest_ids:
                ...
                local_db_logger.info(
                    f"No documents to delete in {self.collection_name} collection."
                )
