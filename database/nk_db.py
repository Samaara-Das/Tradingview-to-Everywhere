...
class Post:
    ...
    def post_to_url(self, payload: dict):
        try:
            ...
            nk_db_logger.exception(
                f"ConnectionError occurred while sending post request to Nishant uncle's webhook "
            )
            return False
        else:
            nk_db_logger.info(
                f"Post request to Nishant uncle's webhook sent successfully! Response: {response}"
            )
            return response
