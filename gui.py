...
class ConfigGUI:
    def __init__(self):
        ...
        self.style.configure(
            "Custom.TEntry", fieldbackground=DarkTheme.ENTRY_BG, foreground=DarkTheme.FG
        )
    ...
    def _create_widgets(self):
        ...
        for field_name, field_type, description in fields:
            ...
            label = tk.Label(
                frame, text=f"{description}:", bg=DarkTheme.BG, fg=DarkTheme.FG
            )
        ...
        status_frame = tk.Frame(
            main_frame, bg=DarkTheme.STATUS_BG, relief="sunken", bd=1
        )
            ...
            main.run_trading_view(
                on_status_change=lambda msg, is_error: self.root.after(
                    ...
                    lambda: self.status_label.config(
                        text=msg, fg=DarkTheme.ERROR if is_error else DarkTheme.SUCCESS
                    ),
                )
            )
    ...
    def _handle_error(self, error_msg):
        ...
        self.status_label.config(text=f"Error: {error_msg}", fg=DarkTheme.ERROR)
        ...
        try:
            ...
            if interval <= 0:
                messagebox.showerror(
                    "Error", "Interval minutes must be a positive number"
                )
                return False
    ...
    def start(self):
        ...
        self.status_label.config(
            text=f"Application started at {current_time}", fg=DarkTheme.SUCCESS
        )
