...
def get_ad_acc_user():
    ...
    response = requests.get(
        f"https://api.linkedin.com/rest/adAccountUsers?q={ACCESS_TOKEN}",
        headers=headers,
    )
    post_info = response.json()
    print(post_info)
...
def fetch_ad_acc():
    ...
    response = requests.get(
        f"https://api.linkedin.com/rest/adAccounts/{AD_ACCOUNT_ID}", headers=headers
    )
    post_info = response.json()
    print(post_info)
...
def init_img_upload():
    ...
    payload = {"initializeUploadRequest": {"owner": ORGANIZATION_URN}}

    response = requests.post(
        f"https://api.linkedin.com/rest/images?action=initializeUpload", headers=headers
    )
