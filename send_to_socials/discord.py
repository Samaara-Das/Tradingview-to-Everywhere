...
class Discord:
    ...
    def send_to_entry_channel(self, category, content):
        try:
            webhook = DiscordWebhook(
                url=self.entry_webhook_urls[category], content=content
            )
            response = webhook.execute()
        except Exception as e:
            discord_logger.exception(
                f'Error sending "{content}" to {category} webhook for the entry channel. Response: {response} Error:'
            )

    def send_to_exit_channel(self, category, content):
        try:
            webhook = DiscordWebhook(
                url=self.exit_webhook_urls[category], content=content
            )
            response = webhook.execute()
        except Exception as e:
            discord_logger.exception(
                f'Error sending "{content}" to {category} webhook for the exit channel. Response: {response} Error:'
            )
    ...
    def send_to_before_and_after_channel(self, category, entry_img, exit_img):
        try:
            ...
            webhook = DiscordWebhook(
                url=self.before_after_webhook_urls[category], content=content
            )
            response = webhook.execute()
        except Exception as e:
            discord_logger.exception(
