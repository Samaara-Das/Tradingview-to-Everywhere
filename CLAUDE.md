...
- `reupload_indicator()` — add fresh copy from Favorites first (text-content match on `data-role="menuitem"`, not hashed CSS classes), confirm on chart, then delete OLD copy, then call `_set_indicator_instance_id()` when `INSTANCE != "tte-1"`, then `Ctrl+S` to persist the new ID into the saved layout. Delete-old reordered before set-id (PR #49) so `_safe_indicator_access` targets the fresh copy unambiguously — the old order silently mis-tagged ~800 alerts on tte-2 on 2026-05-20. Fails closed (returns False) if delete-old throws so the bug can't recreate via a stale-element trigger (PR #49 code review).
...
| Re-upload indicator | `tte/browser/tradingview.py` `reupload_indicator()` | Add fresh → confirm → delete OLD → set ID on remaining copy → `Ctrl+S` to persist. Stable text-match selector instead of hashed CSS classes (PR #48). Delete-then-set ordering + fails-closed on delete failure (PR #49) — prior order silently mis-tagged alerts. |
| Set Instance ID after reupload (+ verify) | `tte/browser/tradingview.py` `_set_indicator_instance_id()` | Restores Pine `Instance ID` input to `TTE_INSTANCE`. Post-submit polls legend text for `instance_id`; if target != `tte-1` and any legend item still shows `tte-1`, fails closed (catches duplicate-on-chart case). PR #48 + PR #49. |
